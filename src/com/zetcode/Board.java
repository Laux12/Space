...
public class Board extends JPanel {
	...
	private List<Alien> aliens;// data structure
	...
	private void gameInit() {
		...
		for (int i = 0; i < 300; i++) {
			for (int j = 0; j < 6; j++) {
				...
				aliens.add(alien);// add the alien to the list
			}
		}
		// add the player
		player = new Player();
	...
	private void drawAliens(Graphics g) {
		...
		for (Alien alien : aliens) {
			// if the alien is visible to the screen then draw the alien
			if (alien.isVisible()) {
	...
	// method for player to draw the shot
	...
	// method for alien to draw the bomb
	private void drawBombing(Graphics g) {
		// every alien will drop a bomb
		for (Alien a : aliens) {
	...
	// paint all the components for the game
	@Override
	...
	// to draw the game over interface
	...
	// to update the player and alien position, kill alien, kill player,player shot
	// and aliens bomb
	private void update() {
		...
		// for aliens to sideward and forward
		for (Alien alien : aliens) {
	...
	// for key listener if we pressed something in our keyboard
